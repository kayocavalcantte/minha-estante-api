"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AutoresService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const autores_entity_1 = require("./entities/autores.entity");
const typeorm_2 = require("typeorm");
let AutoresService = class AutoresService {
    autoresRepository;
    constructor(autoresRepository) {
        this.autoresRepository = autoresRepository;
    }
    async create(createAutoreDto) {
        const autor = this.autoresRepository.create(createAutoreDto);
        return await this.autoresRepository.save(autor);
    }
    async findAll() {
        return await this.autoresRepository.find();
    }
    async findOne(id) {
        const autor = await this.autoresRepository.findOneBy({ id });
        if (!autor) {
            throw new common_1.NotFoundException(`Autor com o ID ${id} não encontrado.`);
        }
        return autor;
    }
    async update(id, updateAutoresDto) {
        const autor = await this.autoresRepository.preload({
            id: id,
            ...updateAutoresDto,
        });
        if (!autor) {
            throw new common_1.NotFoundException(`Autor com o ID ${id} não encontrado.`);
        }
        return this.autoresRepository.save(autor);
    }
    async remove(id) {
        const autor = await this.findOne(id);
        await this.autoresRepository.remove(autor);
    }
};
exports.AutoresService = AutoresService;
exports.AutoresService = AutoresService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(autores_entity_1.Autor)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], AutoresService);
//# sourceMappingURL=autores.service.js.map